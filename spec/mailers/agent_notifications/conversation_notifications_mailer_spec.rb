...
RSpec.describe AgentNotifications::ConversationNotificationsMailer do
  ...
  describe 'conversation_mention' do
    ...
    it 'renders the senders name' do
      expect(mail.body.decoded).to match("Você foi mencionado em uma conversa. <b>#{another_agent.display_name}</b> escreveu:")
    end
  ...
  describe 'participating_conversation_new_message' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, nova mensagem em uma conversa que você participa [ID - #{message.conversation.display_id}].")
    end
