...
- Incoming messages now clear the contact typing state immediately in the realtime client and emit an authoritative `paused` event from the Whatsmeow webhook. The 30-second safety timeout is no longer visible after the contact sends a message.
...
- Presence resolution checks every matching phone/LID `ContactInbox` before selecting the newest active conversation. An orphan alias can no longer hide the canonical contact inbox that owns the conversation.
...
- Whatsmeow attachments enqueue `SendReplyJob` immediately instead of entering Sidekiq's scheduled set. This removes the attachment-only scheduling bottleneck that could turn the intended two-second wait into several minutes on staging.
- If the Sidekiq container cannot read an attachment from its local Active Storage disk, Whatsmeow retrieves the same signed file through the web service before dispatching it. Transient `404 Not Found` responses are retried with a short bounded backoff while the blob becomes available. This keeps both regular audio files and recorded PTT voice notes reliable without giving up immediate queueing when web and worker storage are not mounted together.
...
- Published implementation commit `428a19b166` through successful GitHub Actions run `33936073212`; Chatwoot web and Sidekiq run image digest `sha256:83307acd47c17ec59c280bd9002fd4955a99a4857ddda0acfed76abfc3f8b99c`.
...
- Local validation: Go service tests and the four existing inbox API tests pass; the changed Vue component passes ESLint. The API tests used a local-only PostCSS bypass, jsdom, and Vitest globals for this workstation.
- Published implementation commit `bbe7e71de1` through successful GitHub Actions run `33941085117`; Chatwoot web and Sidekiq run image digest `sha256:b71a2ed949816ab5e1db2f98596c617a3337f41141710334652c16ee48ddda9c`. Whatsmeow was rebuilt from develop and deployed through Easypanel action `cmtnt1kxu001n07mx7jxqdu7y`.
- Live API verification on account 2 / inbox 15 passed: create and force-new returned different QR images; both decoded as the expected linked-devices pairing payload. After the full 160-second cycle, status returned disconnected without a QR and the QR endpoint returned 404. Inbox 12 remained connected after the service restart.
- Live Chrome verification confirms Atualizar QR Code is displayed and two clicks generate different QR images. After the full cycle, the image disappeared, the status changed to Desconectado, the expiry instructions appeared, and Atualizar QR Code / Conectar were enabled. Phone scanning and final authentication still require the user’s device.
- Web and Sidekiq rollout again required temporary stop-first updates due to VPS resource reservations; start-first policy was restored after both services ran the new digest. Chatwoot reports queue/data services OK and Whatsmeow health is healthy.

## September 2026 Performance Audit

- Windows sampling captured a temporary 100% CPU spike with multiple cl.exe compilation processes among the largest consumers. Those processes ended during inspection and total CPU fell to approximately 4%. This is evidence of transient local contention, not a measurement of the Chatwoot tab alone; no Windows processes were stopped or changed.
- The conversation list already uses Virtua virtualization. Live inspection showed about 8–10 rendered cards and 1,323–1,430 DOM elements while changing filters and scrolling. Existing pagination guards prevent overlapping page loads. No rewrite of rendering/pagination was justified.
- The VPS has 2 logical CPUs and about 8 GB RAM. Chatwoot web was limited to 0.75 CPU / 1 GB RAM, while Whatsmeow used approximately 35 MB and little CPU. Across one cgroup sample, 4,145 of 4,708 CPU scheduling periods were throttled (about 88%). No OOM kills were recorded for the web container. These are scoped samples, not proof of every reported slowdown.
- The status polling action previously replaced each inbox on every successful check and marked it disconnected after any network error/timeout. It now retains the last confirmed status on failed/empty responses, commits only changed status/phone values, and applies results to the current record so an in-flight poll cannot overwrite a newer inbox edit. Confirmed disconnections still propagate normally.
- Local validation: 27 existing inbox store action/mutation tests passed with a local-only PostCSS bypass; ESLint passed. An isolated diagnostic exercised unchanged replies, timeouts, confirmed disconnect/reconnect, changed phone numbers, concurrent inbox edits, and empty payloads.
- Published code commit `f5744157e6` through successful GitHub Actions run `33970095357`. Web and Sidekiq both run image `sha256:24a0d8707095526f8904b637010f51b27de7522daeb88c8fc231265b752aabb4`.
- A conservative web CPU limit increase from 0.75 to 1 core was attempted while preserving reservations and memory. Docker Swarm kept web Pending with insufficient resources even after stop-first; restoring the original 0.75-core limit allowed web to start. Easypanel resources were also restored to the saved original values. Further resource redistribution is deferred, rather than expanding the operational change. The site was briefly unavailable during this rollout/recovery, and original start-first update policy was restored afterward.
- Post-deployment API health reports queue/data services OK. Live Chrome filter switching and scrolling passed with no captured console errors. Four Whatsmeow channels displayed connected; Note8busine displayed disconnected, matching its direct service status. The user view was returned to Minhas.
- Follow-up Windows sampling showed approximately 1.1% aggregate CPU across Chrome processes (not an isolated Chatwoot-tab measurement), with 7.6 GB physical memory available. Three direct health requests from this PC took 591 ms initially and 148/168 ms subsequently. These observations do not rule out intermittent network or browser problems; no broad speedup percentage is claimed.

## September 22, 2026 — persistent message history synchronization

- Removed the hard-coded 72-hour import cutoff. Each inbox defaults to a 90-day automatic window and supports a custom start date, manual sync, automatic import preferences and pause. New pairings request extended history from WhatsApp (up to 365 days / 1 GB, subject to phone availability).
- Go persists protobuf history before importing, uses one bounded worker, retries failed webhooks, keeps per-chat paging anchors and requests 50 older messages per page. Manual resync starts from the newest known message to recover gaps, and duplicate messages stay idempotent. Raw imported copies expire after seven days; unprocessed raw copies after 30 days. This does not delete Chatwoot messages. Existing attachment retention remains in force.
- Settings and sidebar show actual received/processed/pending counts and distinguish waiting, partial response, pause and errors. No percentage or completeness claim is inferred from a request acknowledgement. The phone may not provide old history/media; unknown conversations without a history chunk or known message anchor cannot be requested individually.
...
## September 2026 — MX and principal sync/media follow-up

- This repository builds the shared fork used by `chatwoot.marcoswt.com.br` and `chatwootmx.marcoswt.com.br`. Publish matching Chatwoot images to both web/Sidekiq pairs and matching Go code to both Whatsmeow services. Each instance keeps its own database and WhatsApp session.
- The earlier MX count of roughly 13,000 represented raw history records processed across only 14 Chatwoot conversations. It did not measure WhatsApp conversation coverage. The Go worker now reports actual Chatwoot message and conversation counts for the chosen period, and the UI labels received/processed records separately in Portuguese.
- Manual sync retries cached history records that have no Chatwoot message, including previously skipped self-chat records. The self-chat contact is now resolved explicitly as `Mensagens para mim`; older-history cursors use the oldest known message, and failed requests release their cursor for retry.
- Message controls now support a two-second primary-button hold to select, a single ZIP download for several selected attachments, compact image/video bubbles, grouping of adjacent photos, thumbnail gallery navigation, image reaction/forward/save controls, and the full existing emoji picker.
- WhatsApp sends the initial chat inventory on a new pairing. Older pages can only be requested for known chats; the sync UI must never claim that all phone chats were imported just because the local queue emptied. Re-pair MX only after matching code is deployed, then compare known conversations and a recent outbound message against the phone.
- Live MX verification on September 23: Marco paired a new device (`:26`) and the app reported Connected. The stored count rose from about 13,000 messages in 14 conversations before the repair to 15,275 messages in 52 conversations while 14,396 received records remained queued. Newly imported conversations appeared in the dashboard, so the activity indicator reflects actual message creation rather than only raw processing. Leave the import running; these figures are a point-in-time observation, not a completeness claim.
- Live UI verification showed grouped photos in one compact bubble, a gallery with thumbnail navigation and Portuguese actions, and a selection toolbar with download for two selected photos. The ZIP action reported success in the browser. The final locale pass places download result keys under `CONVERSATION.MESSAGE_SELECTION` and translates emoji category headings; after deployment, refresh both instances and confirm those strings in Portuguese.
- A live check of `Mensagens Para Mim` exposed a backlog ordering issue: the worker selected the oldest pending raw record first, leaving the phone's recent self-chat messages queued behind thousands of older entries. Pending imports now prioritize newest timestamps. Already received records stay in the database, and the worker continues importing the rest afterward.

## September 24, 2026 — read state, conversation order, and inbox controls

- The Whatsmeow service forwards linked-device `MarkChatAsRead` actions and `read-self` receipts to Chatwoot. Chatwoot updates the conversation read cursor to the WhatsApp message timestamp, preserving newer unread messages, and broadcasts conversation and unread-count updates. A token-protected `read-state/sync` endpoint requests the current WhatsApp app-state snapshot for an already connected inbox; this is intended for one-time reconciliation after rollout.
- Delayed WhatsApp messages no longer lower a conversation's `last_activity_at`. The dashboard also sorts its loaded conversation list by the selected sort mode after filtering, so message arrival order in Vuex cannot determine display order.
- Each user's pinned conversation IDs are saved in account-scoped UI settings. Up to three appear first; pinning a fourth removes the oldest pin. The right-click channel menu now has an admin-only Excluir action using the existing typed-name confirmation.
- The quick reaction row is slightly smaller. It and the full emoji picker render above the conversation/contact columns through a body teleport with viewport-aware placement.
- Validate new read-state events on MX first. The WhatsApp read marker and Chatwoot unread badge may differ for unsupported WhatsApp surfaces or messages outside the imported history; do not describe a completed app-state fetch as full message-history parity.

## September 25, 2026 — persistent dashboard layout and filters

- The conversation sidebar width, other panel widths and visible conversation tabs are per-user UI settings. UI writes now send only changed keys; browser writes are serialized and the profile controller merges patches while holding the user row lock. This prevents simultaneous drag and filter updates from restoring an older settings snapshot.
- Conversation filters apply automatically after a valid change. The Apply button is removed; Clear remains. The normal conversation tabs and header remain visible while a filter is active, and the list respects the selected assignee and group visibility.
- The change was merged through PR #9 (`dfad7763e9`) and published as `ghcr.io/marcosenz0/chatwoot-whatsmeow@sha256:1bf2543d8b27fd95110078a2cf0d206b60c0bd0bc2a25034adb17b1f3f10535e` to both custom domains. Each web and Sidekiq deployment succeeded; `chatwootoficial.marcoswt.com.br` was untouched.
- Live checks after reload: MX retained only Todos and Grupos; the principal site retained its adjusted conversation-list width and visible tabs. Both filter panels showed Limpar filtros without Aplicar filtros, and both public domains returned HTTP 200. Recent MX messages were visible, so this task made no WhatsApp sync pipeline changes; this observation is not a full phone-to-Chatwoot parity audit.
- Local frontend validation passed 28 focused tests, formatting and ESLint errors (with three existing dynamic-i18n warnings). Ruby syntax passed. Local RSpec was unavailable on Windows; repository-wide CI reported unrelated pre-existing Ruby lint and backend spec failures, while the frontend CI passed.
